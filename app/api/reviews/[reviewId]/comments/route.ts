...
import { sendNewReviewCommentNotifications } from "@/lib/email";
...
export async function POST(request: Request, { params }: { params: Promise<{ reviewId: string }> }) {
  try {
    ...
    if (!authorName || !email.includes("@") || !commentBody) return NextResponse.json({ error: "Please enter your name, email and comment." }, { status: 400 });
    if (!isDbConfigured) return NextResponse.json({ error: "Discussion is temporarily unavailable." }, { status: 503 });
    const review = await db.testimonial.findFirst({ where: { id: reviewId, source: "REVIEWS_CENTER", isActive: true }, select: { id: true, authorName: true, email: true } });
    ...
    await db.reviewComment.create({ data: { reviewId, parentId, authorName, email, body: commentBody, isActive: true } });
    await sendNewReviewCommentNotifications({ authorName, clientEmail: email, body: commentBody, reviewAuthorName: review.authorName, reviewAuthorEmail: review.email ?? undefined, reviewId, isReply: Boolean(parentId) });
